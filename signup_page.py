...
class SignUp:
    def __init__(self, root):
        ...
        frame.place(x=195,y=15,width=500,height=550)
        ...
        self.fname_txt = Entry(frame,font=("arial"),borderwidth=2)
        self.fname_txt.place(x=20, y=130, width=200)

        self.lname_txt = Entry(frame,font=("arial"),borderwidth=2)
        self.lname_txt.place(x=240, y=130, width=200)

        email = Label(frame, text="Email", font=("helvetica",15,"bold"),bg="white").place(x=20, y=180)

        self.email_txt = Entry(frame,font=("arial"),borderwidth=2)
        self.email_txt.place(x=20, y=210, width=420)

        sec_question = Label(frame, text="Select Question", font=("helvetica",15,"bold"),bg="white").place(x=20, y=260,)
        ...
        self.answer_txt = Entry(frame,font=("arial"),borderwidth=2)
        ...
        self.password_txt = Entry(frame,font=("arial"),borderwidth=2)
    ...
    def signup_func(self):
        ...
        if self.fname_txt.get()=="" or self.lname_txt.get()=="" or self.email_txt.get()=="" or self.questions.get()=="Select" or self.answer_txt.get()=="" or self.password_txt.get() == "":
            messagebox.showerror("Omega Studio","Sorry!, All fields are required",parent=self.window)
            ...
            try:
                ...
                if row!=None:
                    messagebox.showerror("Omega Studio","The email id is already exists, please try again with another email id",parent=self.window)
                else:
                    ...
                    connection.commit()
                    connection.close()
                    messagebox.showinfo("Omega Studio","Register Successful",parent=self.window)
                    self.reset_fields()
                    self.window.destroy()
            except Exception as es:
                messagebox.showerror("Omega Studio",f"Error due to {es}",parent=self.window)
